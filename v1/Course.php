...
class Course
{
    ...
    public function addData($json) {
        ...
        $sql = "INSERT INTO pel_edu_courses (faculty_name,faculty_id,course_name,course_code,added_date,verified_date,added_by,verified_by) VALUES "
            . "(:faculty_name,:faculty_id,:course_name,:course_code,now(),now(),:added_by,:verified_by)";
    ...
    public function updateData($json) {
        ...
        $course_id = isset($json->course_id) ? $json->course_id : "";
        $course_name = isset($json->course_name) ? $json->course_name : "";
        $course_code = isset($json->course_code) ? $json->course_code : "";
        $added_by = isset($json->added_by) ? $json->added_by : "";
        $verified_by = $added_by;

        $sql = "UPDATE pel_edu_courses SET course_name = :course_name,course_code = :course_code, verified_by = :verified_by WHERE course_id = :course_id ";
        $params = [
            ':course_id' => $course_id,
            ':course_name' => $course_name,
            ':course_code' => $course_code,
    ...
    public function removeData($id) {
        ...
        $sql = "DELETE FROM pel_edu_courses WHERE course_id = :id";
        $params = [
            ':id' => $id
        ];
    ...
    public function getTable() {
        ...
        if($filter && strlen($filter) > 1 ) {
            ...
            $orWhere[] = "pel_edu_courses.course_name regexp :filter ";
            $params[':filter'] = $filter;
        }

        if($faculty_id && strlen($faculty_id) > 1 ) {

            $orWhere[] = "pel_edu_courses.faculty_id = :faculty_id ";
