...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "checkindb")) {
	if (isset($_POST['identity_number']) || isset($_POST['license_number'])) {
		$identity_number = $_POST['identity_number'];
		$license_number = $_POST['license_number'];
		mysqli_select_db($connect,$database_connect);
		$datetoday = date ('Y-m-d');
		$query_getstudent = "SELECT * FROM pel_individual_psv_data WHERE (identity_number = '".$identity_number."' OR license_number = '".$license_number."') AND expiry_date > '".$datetoday."' AND status = '11'";
		...
		if($totalRows_getstudent > 0) {
			echo $insertSQL = sprintf("INSERT INTO pel_individual_psv_data (first_name, identity_number, status, date_added, added_by, expiry_date, data_source, data_notes, psv_photo, license_number, psv_photo, psv_status, country, date_of_issue, search_id) VALUES (%s, %s, %s, now(), %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
												...
												GetSQLValueString(strtoupper($row_getstudent['psv_photo']), "text"),
												GetSQLValueString(strtoupper($row_getstudent['license_number']), "text"),
												GetSQLValueString($row_getstudent['psv_photo'], "text"),
												GetSQLValueString($row_getstudent['psv_status'], "text"),
												GetSQLValueString($row_getstudent['country'], "text"),
												GetSQLValueString($row_getstudent['date_of_issue'], "text"),
			...
			if (mysqli_error($connect)) {
				$errorcode = '<div class="alert alert-danger">
											...
											Details of the PSV Data havent been added.
												<br />
											</div>';
			} else { 
				$updateGoTo = "psvlicensecheck.php?request_id=$colname_getrequestid&moduleid=$colname_getmoduleid";
			...
			$errorcode = '<div class="alert alert-danger">
										 ...
										 NO Previous Details of PSV found
											<br />
										</div>';
		}
	}
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "editdetails")) {
	if (is_uploaded_file($_FILES['psv_photo']['tmp_name'])) {
	date_default_timezone_set('Africa/Nairobi');
	$date_insert = date('dmYhis');
	$a = $_POST['license_number']."-".$_POST['search_id']."-".$date_insert; 
	$rawname = $_FILES['psv_photo']['name'];
		"Upload: ".$a."_". $_FILES["psv_photo"]["name"];
		$file="psvphotos/".$a."_". $_FILES["psv_photo"]["name"];

		require_once "../../uploads.php";
		$prefix = "psv_photo";
		$filenameuploaded = uploadFile($prefix,"individual-psvphotos",$a."_".$_FILES[$prefix]["name"]);
	} else {
		$filenameuploaded = $_POST['psv_photo2']; 
	}
  $updateSQL = sprintf("UPDATE pel_individual_psv_data SET first_name=%s, identity_number=%s, status=%s, date_added=%s, added_by=%s, expiry_date=%s, data_source=%s, data_notes=%s, operator_license=%s, license_number=%s, psv_photo=%s, psv_status=%s, country=%s, date_of_issue=%s WHERE psv_id=%s",
											...
											GetSQLValueString($_POST['operator_license'], "text"),
											GetSQLValueString($_POST['license_number'], "text"),
											GetSQLValueString($filenameuploaded, "text"),
											GetSQLValueString($_POST['psv_status'], "text"),
											GetSQLValueString($_POST['country'], "text"),
											GetSQLValueString($_POST['date_of_issue'], "text"),
											GetSQLValueString($_POST['psv_id'], "int"));
  ...
  if (mysqli_error($connect)) {
		$errorcode = '<div class="alert alert-danger">
											...
											<strong>
											 ...
											 Details of the PSV were not updated succesfully.
											<br />
										</div>';
	} else {
		$updateGoTo = "psvlicensecheck.php?request_id=$colname_getrequestid&moduleid=$colname_getmoduleid";
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "newdetails")) {
	if (is_uploaded_file($_FILES['psv_photo']['tmp_name'])) {
		...
		$insertSQL = sprintf("INSERT INTO pel_individual_psv_data (first_name, identity_number, status, date_added, added_by, expiry_date, data_source, data_notes, operator_license, license_number, psv_photo, psv_status, country, date_of_issue, search_id) VALUES (%s, %s, %s, now(), %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
											GetSQLValueString(strtoupper($_POST['first_name']), "text"),
											GetSQLValueString(strtoupper($_POST['identity_number']), "text"),
											GetSQLValueString($_POST['status'], "text"),
											GetSQLValueString($_POST['added_by'], "text"),
											...
											GetSQLValueString($_POST['data_source'], "text"),
											GetSQLValueString($_POST['data_notes'], "text"),
											GetSQLValueString(strtoupper($_POST['operator_license']), "text"),
											GetSQLValueString(strtoupper($_POST['license_number']), "text"),
											GetSQLValueString($filenameuploaded, "text"),
											GetSQLValueString($_POST['psv_status'], "text"),
											GetSQLValueString($_POST['country'], "text"),
											GetSQLValueString($_POST['date_of_issue'], "text"),
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "addreject")) {
	$deleteSQLreject = sprintf("UPDATE pel_individual_psv_data SET status=%s, review_status=%s, verified_by=%s, verified_date=now(), review_notes=%s WHERE search_id=%s",
...
if ((isset($_GET['psv_id'])) && ($_GET['psv_id'] != "")) {
	if($_GET['status']=='00') {
  	$deleteSQL = sprintf("UPDATE pel_individual_psv_data SET status=%s, added_by=%s, date_added=now() WHERE psv_id=%s",
											GetSQLValueString($_GET['status'], "text"),
											GetSQLValueString($_GET['fullnames'], "text"),
					   		 			GetSQLValueString($_GET['psv_id'], "int"));
		mysqli_select_db($connect,$database_connect);
...
if ((isset($_GET['search_id_approve'])) && ($_GET['search_id_approve'] != "")) {
	if($_GET['status']=='11') {
   	$deleteSQL2 = sprintf("UPDATE pel_individual_psv_data SET status=%s, review_status=%s, verified_by=%s, verified_date=now() WHERE search_id=%s",
											GetSQLValueString($_GET['status'], "text"),
...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Individual PSV Details Data Management - Peleza Admin</title>
					...
					<div class="breadcrumbs" id="breadcrumbs">
					  <script type="text/javascript">
						...
						<ul class="breadcrumb">
							...
							<li class="active">Individual PSV License Details</li>
						</ul><!-- /.breadcrumb -->
					...
					<div class="page-content">
						<div class="row">
							<div class="col-xs-12">
								...
								<div class="row">
									<div class="col-xs-12">
									  <h3 align="left" class="header smaller lighter blue">Add Individual PSV Details</h3>
									</div>
									<?php
	  							...
	  							<h3 align="left" class=" smaller lighter blue"><strong>SEARCH REF: </strong><?php echo $row_getstudent['request_ref_number'];?></h3>
										<div>
											...
											<?php
												...
												$query_getdetails = "SELECT * FROM pel_individual_psv_data WHERE search_id = '".$search_ref."'";
											...
											?> 
												<div class="col-lg-12" align="center">
													<h3 align="left" class=" smaller lighter blue"><strong>INDIVIDUAL PSV CHECK DETAILS: </strong></h3>
													<table id="simple-table" class="table table-bordered table-hover">
														<thead>
															<tr>									
																<th>HOLDERS NAME:</th>
																<th>IDENTITY NUMBER:</th> 
																<th>OPERATOR LICENSE:</th>
														...
														<tr>
															<td><a href="#"><?php echo $row_getdetails['first_name']; ?> </a></td>
															<td><?php echo $row_getdetails['identity_number']; ?></td> 
															<td><?php echo $row_getdetails['operator_license']; ?></td>
															<td><?php echo $row_getdetails['license_number']; ?></td>  
															<td><?php echo $row_getdetails['country']; ?></td>
															<td><?php echo $row_getdetails['data_source']; ?></td>
															<td><?php echo $row_getdetails['date_of_issue']; ?></td> 
															<td><?php echo $row_getdetails['expiry_date']; ?></td>  
															<td>
																<?php 
																	if($row_getdetails['psv_status']=='VALID') {
																		?>
																			<span class="label label-sm label-success">VALID</span>	
																		<?php
																	}
																	if($row_getdetails['psv_status']=='INVALID') {
																		?>
																			<span class="label label-sm label-danger">INVALID</span>	
																		<?php
																	}
																	if($row_getdetails['psv_status']=='NOT AVAILABLE') {
																		?>
																			<span class="label label-sm label-info">NOT AVAILABLE</span>	
																		<?php
																	}
																	if($row_getdetails['psv_status']=='EXPIRED') {
																		?>
																...
																<td class="hidden-480">
																	...
																	<a href="#modal-editdetails-<?php echo $row_getdetails['psv_id']; ?>" role="button" class="green" data-toggle="modal">	
                                 		<button class="btn btn-xs btn-info">  
																			<i class="ace-icon fa fa-pencil bigger-120"></i>	
																		</button>
																	</a>

   																<div id="modal-editdetails-<?php echo $row_getdetails['psv_id']; ?>" class="modal fade" tabindex="-1">
                                            ...
                                            <input type="hidden" id="request_id" name="request_id" value="<?php echo $colname_getrequestid; ?>"/>
                                      			<input type="hidden" id="search_id" name="search_id" value="<?php echo $search_ref; ?>"/>
																						...
																						<input type="hidden" id="psv_id" name="psv_id" value="<?php echo $row_getdetails['psv_id']; ?>"/>
                                        		<input type="hidden" id="status" name="status" value="22"/>
           																	...
           																	<input type="hidden" id="date_added" name="date_added" value="<?php echo date('Y-m-d H:m:s');?>"/>
																					  ...
																					  <div class="col-sm-8">
																							<span id="sprytextfield4">
																								<input  type="text" value="<?php echo $row_getdetails['date_of_issue']; ?>" id="date_of_issue" name="date_of_issue"  required/>
																								<span class="textfieldRequiredMsg">*</span>
																							</span>
																						</div>
																						<br/>
																						...
																						<div class="col-sm-8">
																							<span id="sprytextfield4">
																								<input  type="text" value="<?php echo $row_getdetails['expiry_date']; ?>" id="expiry_date" name="expiry_date"  required/>
																								<span class="textfieldRequiredMsg">*</span>
																							</span>
																						</div>
																						<br/>
																						<div class="space-10"></div>
																						<label class="col-sm-4">Operator License</label>
																						<div class="col-sm-7">
																							<span id="spryselect4">
																								<select class="chosen-select form-control"  name="operator_license" id="operator_license"  data-placeholder="Choose Operator License..." required>
																									<option value="<?php echo $row_getdetails['operator_license']; ?>"><?php echo $row_getdetails['operator_license']; ?></option>
																									<option value="">Operator License</option>
																									<option value="TAXI">TAXI</option>
																									<option value="MATATU">MATATU</option>
																									<option value="LONG DISTANCE BUSES">LONG DISTANCE BUSES</option>
																									<option value="BODA BODA">BODA BODA</option>
																									<option value="TUK TUK">TUK TUK</option>
																									<option value="TOURIST CHAUFFER">TOURIST CHAUFFER</option>
																								</select>
																						...
																						<label class="col-sm-4">License Number</label>
																							<div class="col-sm-7">
																								<span id="sprytextfield5">
																									<input  type="text" value="<?php echo $row_getdetails['license_number']; ?>" id="license_number" name="license_number"  required/>
																									<span class="textfieldRequiredMsg">*</span>
																								</span>
																							</div>
																							<br/>	
																						  ...
																						  <div class="col-sm-7">
																							...
																							<br/>
																							<div class="space-10"></div>
																								<label class="col-sm-4">PSV Status</label>
																									<div class="col-sm-7">
																										<span id="spryselect3">
																											<select class="chosen-select form-control"  name="psv_status" id="psv_status"  data-placeholder="Choose PSV Status..." required>
																												<option  value="<?php echo $row_getdetails['psv_status']; ?>"><?php echo $row_getdetails['psv_status']; ?></option>
														        										<option value="">PSV Status</option>
																									    ...
																									    <span class="selectInvalidMsg">*</span><span class="selectRequiredMsg">*</span></span></div>
																											...
																											<label class="col-sm-4">Data Source</label>
																												...
																												<br/>
												                    						<div class="space-10"></div>
																												<label class="col-sm-4">PSV Photo</label>
																												<div class="col-sm-8">
																													Currennt Photo <br/>
																													<img width="200px" height="200px" src="<?php echo $row_getdetails['psv_photo']; ?>" alt="PSV Photos">
                                                  				<input id="psv_photo2" name="psv_photo2" type="hidden" value="<?php echo $row_getdetails['psv_photo']; ?>"/>    
											  																</div>
                                               					<br/>
																												<div class="space-10"></div> 
																												<label class="col-sm-4">Click to Change Image</label>
																												<div class="col-sm-8">
																													<input class="form-control" id="psv_photo" name="psv_photo" type="file"/>
																												</div>
																												<br/>				
																												<div class="space-10"></div>
                                						...
                                						if($row_getdetails['status']=='11' || $row_getdetails['status']=='22') {
																							?>
																								<a href="psvlicensecheck.php?fullnames=<?php echo $_SESSION['MM_full_names']."(".$_SESSION['MM_USR_EMAIL'].")"; ?>&status=00&psv_id=<?php echo $row_getdetails['psv_id']; ?>&request_id=<?php echo $colname_getrequestid; ?>&moduleid=<?php echo $colname_getmoduleid; ?>">
                                						...
                                						if($row_getdetails['status']=='00') {
																							?>
																								<a href="psvlicensecheck.php?fullnames=<?php echo $_SESSION['MM_full_names']."(".$_SESSION['MM_USR_EMAIL'].")"; ?>&status=22&psv_id=<?php echo $row_getdetails['psv_id']; ?>&request_id=<?php echo $colname_getrequestid; ?>&moduleid=<?php echo $colname_getmoduleid; ?>">
																		...
																		<tr>
																			<td colspan="5"><img width="400px" src="<?php echo $row_getdetails['psv_photo']; ?>"></td>
																			<td colspan="5">
                                          	...
                                          	if($row_getdetails['review_status']=='REJECTED'){                
           																		?>                                               
																								<tr>
																									<td colspan="6">
                                   									<?php echo $row_getdetails['review_notes']; ?>
																									</td>
																								</tr>
																							<?php
																						}
																...
																?> 
                          			<i class="ace-icon fa fa-hand-o-right icon-animated-hand-pointer blue"></i>
																	<a href="psvlicensecheck.php?fullnames=<?php echo $_SESSION['MM_full_names']."(".$_SESSION['MM_USR_EMAIL'].")"; ?>&status=11&search_id_approve=<?php echo $search_ref; ?>&request_id=<?php echo $colname_getrequestid; ?>&moduleid=<?php echo $colname_getmoduleid; ?>">
																		<button class="btn btn-xs btn-success">
																			<i class="ace-icon fa fa-check bigger-120">Approve</i>
																		</button>
																	</a>
																	<i class="ace-icon fa fa-hand-o-right icon-animated-hand-pointer blue"></i>
																		<a href="#modal-addreject" role="button" class="blue" data-toggle="modal">	
                                 			<button class="btn btn-xs btn-danger"> Reject With Reason</button>
																		</a>
	                                              ...
	                                              <div class="clearfix form-actions">
															...
															?>
														<i class="ace-icon fa fa-hand-o-right icon-animated-hand-pointer blue"></i>
														<a href="#modal-checkindb" role="button" class="green" data-toggle="modal">	
															<button class="btn btn-xs btn-success">
														...
														<a href="#" role="button" class="orange">	
															<button class="btn btn-xs btn-warning">
											 	 				<i class="ace-icon smaller-80 green"></i>Fetch From NTSA
															</button>
														</a> 
														<i class="ace-icon fa fa-hand-o-right icon-animated-hand-pointer blue"></i>
														<a href="#modal-adddetails" role="button" class="green" data-toggle="modal">	
															<button class="btn btn-xs btn-primary">
											...
											?>
											<div id="modal-checkindb" class="modal fade" tabindex="-1">
												<div class="modal-dialog">
													<div class="modal-content">
														<div class="modal-header no-padding">
															<div class="table-header">
														...
														<div class="modal-body padding">
															<form method="POST" action="<?php echo $editFormAction; ?>" class="form-horizontal" name="checkindb" >
																<input type="hidden" id="request_id" name="request_id" value="<?php echo $colname_getrequestid; ?>"/>
																<input type="hidden" id="moduleid" name="moduleid" value="<?php echo $colname_getmoduleid; ?>"/>
																<input type="hidden" id="search_id" name="search_id" value="<?php echo $search_ref; ?>"/>
																<input type="hidden" id="status" name="status" value="22"/>
           											...
           											<input type="hidden" id="date_added" name="date_added" value="<?php echo date('Y-m-d H:m:s');?>"/>
																<div class="space-10"></div>
																<label class="col-sm-4">Enter License Number</label>
																<div class="col-sm-8">
								...
								<div class="modal-body padding">
		                            ...
		                            <br/>	
															  ...
															  <div class="col-sm-7">
																...
																<div class="col-sm-8">
																	<input class="form-control" id="psv_photo" name="psv_photo" type="file" required/>
																</div>
																<br/>					
																<div class="space-10"></div>
																<label class="col-sm-4">Comments</label>
