...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "checkindb")) {
	if (isset($_POST['identity_number']) || isset($_POST['tax_number'])) {
		...
		$query_getstudent = "SELECT * FROM pel_individual_tax_data WHERE (identity_number = '" . $identity_number . "' OR tax_number = '" . $tax_number . "') AND expiry_date > '" . $datetoday . "' AND status = '11'";
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "editdetails")) {
	if (is_uploaded_file($_FILES['tax_photo']['tmp_name'])) {
		...
		$file = "taxcompliance/" . $a . "_" . $_FILES["tax_photo"]["name"];

		require_once "../../uploads.php";
		$prefix = "tax_photo";
		$filenameuploaded = uploadFile($prefix, "individual-taxcompliance", $a . "_" . $_FILES[$prefix]["name"]);
	} else {
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "addreject")) {
	$deleteSQLreject = sprintf(
		"UPDATE pel_individual_tax_data SET status=%s, review_status=%s, verified_by=%s, verified_date=now(), review_notes=%s WHERE search_id=%s",
...
if ((isset($_GET['tax_compliance_id'])) && ($_GET['tax_compliance_id'] != "")) {
	if ($_GET['status'] == '00') {
		$deleteSQL = sprintf(
			"UPDATE pel_individual_tax_data SET status=%s, added_by=%s, date_added=now() WHERE tax_compliance_id=%s",
...
if ((isset($_GET['search_id_approve'])) && ($_GET['search_id_approve'] != "")) {
	if ($_GET['status'] == '11') {
		$deleteSQL2 = sprintf(
			"UPDATE pel_individual_tax_data SET status=%s, review_status=%s, verified_by=%s, verified_date=now() WHERE search_id=%s",
			GetSQLValueString($_GET['status'], "text"),
...
<head>
	...
	<meta charset="utf-8" />
	<title>Individual Tax Compliance Details Data Management - Peleza Admin</title>
			...
			<div class="main-content-inner">
				...
				<div class="breadcrumbs" id="breadcrumbs">
					...
					<ul class="breadcrumb">
						...
						<li class="active">Individual Tax Compliance Details</li>
					</ul><!-- /.breadcrumb -->
						...
						<div class="col-xs-12">
							<div class="row">
								...
								<div>
									...
									$query_getdetails = "SELECT * FROM pel_individual_tax_data WHERE search_id = '" . $search_ref . "'";
										...
										<div class="col-lg-12" align="center">
											...
											<table id="simple-table" class="table table-bordered table-hover">
												...
												<tr>
													...
													<td><?php echo $row_getdetails['tax_number']; ?></td>
													...
													<td class="hidden-480">
														...
														<div id="modal-editdetails-<?php echo $row_getdetails['tax_compliance_id']; ?>" class="modal fade" tabindex="-1">
																...
																<div class="modal-content">
																	...
																	<div class="modal-body padding">
																		<form enctype="multipart/form-data" method="POST" action="<?php echo $editFormAction; ?>" class="form-horizontal" name="editdetails">
																			...
																			<label class="col-sm-4">Tax Organisation</label>
																			...
																			<div class="col-sm-7"><span id="sprytextfield5">
																					<input type="text" value="<?php echo $row_getdetails['tax_number']; ?>" id="tax_number" name="tax_number" required />
																					<span class="textfieldRequiredMsg">*</span></span></div>
																			...
																			<label class="col-sm-4">Tax Status</label>

																			<div class="col-sm-7"><span id="spryselect3">
																					<select class="chosen-select form-control" name="tax_status" id="tax_status" data-placeholder="Choose Tax Status..." required>
																						<!--  <select name="client_industry" id="client_industry">-->
																						<option value="<?php echo $row_getdetails['tax_status']; ?>"><?php echo $row_getdetails['tax_status']; ?></option>
																			...
																			<label class="col-sm-4">Tax Photo</label>
														...
														?>
															<a href="taxcompliancecheck.php?fullnames=<?php echo $_SESSION['MM_full_names'] . "(" . $_SESSION['MM_USR_EMAIL'] . ")"; ?>&status=00&tax_compliance_id=<?php echo $row_getdetails['tax_compliance_id']; ?>&request_id=<?php echo $colname_getrequestid; ?>&moduleid=<?php echo $colname_getmoduleid; ?>"> <button class="btn btn-xs btn-danger">
																	<i class="ace-icon fa fa-trash-o bigger-120"></i> </button></a> <?php
																																																}
																																																if ($row_getdetails['status'] == '00') {

																																																	?>
															<a href="taxcompliancecheck.php?fullnames=<?php echo $_SESSION['MM_full_names'] . "(" . $_SESSION['MM_USR_EMAIL'] . ")"; ?>&status=22&tax_compliance_id=<?php echo $row_getdetails['tax_compliance_id']; ?>&request_id=<?php echo $colname_getrequestid; ?>&moduleid=<?php echo $colname_getmoduleid; ?>"> <button class="btn btn-xs btn-success">
									...
									?>
										...
										<i class="ace-icon fa fa-hand-o-right icon-animated-hand-pointer blue"></i><a href="#" role="button" class="orange">
											<button class="btn btn-xs btn-warning">
												<i class="ace-icon smaller-80 green"></i>Fetch From Tax Organisation
											</button></a>
											...
											<div class="modal-content">
												...
												<div class="modal-body padding">
													<form method="POST" action="<?php echo $editFormAction; ?>" class="form-horizontal" name="checkindb">
														...
														<div class="space-10"></div>
														<label class="col-sm-4">Enter Tax Number</label>

														<div class="col-sm-8"><input type="text" id="tax_number" name="tax_number" />
														</div>
											...
											<div class="modal-content">
												...
												<div class="modal-body padding">
													<form enctype="multipart/form-data" method="POST" action="<?php echo $editFormAction; ?>" class="form-horizontal" name="newdetails">
														...
														<div class="col-sm-7"><span id="sprytextfield5">
																<input type="text" id="tax_organisation" name="tax_organisation" required />
																<span class="textfieldRequiredMsg">*</span></span></div>
