...
if ((isset($_GET['package_id'])) && ($_GET['package_id'] != "")) {
	...
	if ($_GET['package_status'] == '11') {
		...
		$deleteSQL = sprintf(
			"UPDATE pel_package SET package_status=%s, package_verified_by=%s, package_verified_date=%s WHERE package_id=%s",
	...
	if ($_GET['package_status'] == '00' || $_GET['package_status'] == '22') {
		...
		$deleteSQL = sprintf(
			"UPDATE pel_package SET package_status=%s, package_added_by=%s, package_added_date=%s WHERE package_id=%s",
			GetSQLValueString($_GET['package_status'], "text"),
			GetSQLValueString($_GET['fullnames'], "text"),
			GetSQLValueString(date('d-m-Y H:m:s'), "text"),
			GetSQLValueString($_GET['package_id'], "int")
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "newPackage")) {
	$insertSQL = sprintf(
		"INSERT INTO pel_package (package_name, package_cost, package_status, package_added_by, package_added_date, package_data, package_currency, package_min, package_max, package_credits, dataset_id) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "editPackage")) {
	$updateSQL = sprintf(
		"UPDATE pel_package SET package_name=%s, package_cost=%s, package_status=%s, package_added_by=%s, package_added_date=%s, package_data=%s, package_currency=%s, package_min=%s, package_max=%s, package_credits=%s, dataset_id=%s WHERE package_id=%s",
		GetSQLValueString(strtoupper($_POST['package_name']), "text"),
		GetSQLValueString($_POST['package_cost'], "text"),
		GetSQLValueString($_POST['package_status'], "text"),
		GetSQLValueString($_POST['package_added_by'], "text"),
							...
							<div class="row">
								<div class="col-xs-12">
											...
											<h3 align="right" class="header smaller lighter blue">
												...
												<a href="assignpackagemodules.php" role="button" class="green">
													<button class="btn btn-success btn-info btn-bold">
														<i class="ace-icon bigger-120 green"></i>Assign Modules</button>
												</a>
											</h3>
										</div>
									<?php
									...
									<div class="table-header">
										Results for "Packages configured"
									</div>
									<div>
											...
											<tbody>
												...
												do { ?>
													<tr>
														...
														<td>
															<a href="#"><?php echo $row_getallPackages['package_name']; ?></a>
														</td>
													...
													<div id="modal-editPackage-<?php echo $row_getallPackages['package_id']; ?>" class="modal fade" tabindex="-1">
														<div class="modal-dialog">
															<div class="modal-content">
																...
																<div class="modal-body padding">
																	<form method="POST" action="<?php echo $editFormAction; ?>" class="form-horizontal" name="editPackage">
